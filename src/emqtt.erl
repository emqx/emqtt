...
- `{name, Name}` :  If a name is provided, the gen_statem will be registered with this name.
For details see the documentation for the first argument of `gen_statem:start_link/4`.


- `{owner, Pid}`: Client process will send messages like `{diconnected, ReasonCode, Properties}` to the owner process.

- `{host, Host}`: The host of the MQTT server to be connected. Host can be a hostname or an IP address. Defaults to `localhost`

- `{hosts, [{Host, Port}]}`: A list of hosts to connect to. If the connection to the first host fails, the client will try the next host in the list. If the connection to all hosts fails, the client will return an error. Setting this option will override the `host` option.

- `{port, Port}`: The port of the MQTT server to be connected. If not given, the default of 1883 for MQTT or 8883 for MQTT over TLS will be used.

- `{tcp_opts, Options}`: Additional options for `gen_tcp:connect/3`.

- `{ssl, boolean()}`: Enable SSL/TLS transport or not. Defaults to false.

- `{ssl_opts, Options}`: Additional options for `ssl:connect/3`.

- `{ws_path, Path}`: Path to the resource. Defaults to `/mqtt`

- `{connect_timeout, Timeout}`: The maximum time to wait to connect to the server and the server returns a CONNACK. Defaults to 60s.

- `{bridge_mode, boolean()}`: Enable bridge mode or not. Defaults to false.

- `{clientid, ClientID}`: Specify the client identifier. If not given, the client identifier will use the value assigned by the server in MQTT v5 or be automatically generated by internal code in MQTT v3.1/v3.1.1.

- `{clean_start, CleanStart}`: Whether the server should discard any existing session and start a new session. Defaults to true.

- `{username, Username}`: Username used by the server for authentication and authorization.

- `{password, Password}`: Password used by the server for authentication and authorization.

- `{proto_ver, ProtocolVersion}`: MQTT protocol version. Defaults to `v4`.

- `{keepalive, Keepalive}`: Maximum time interval that is permitted to elapse between the point at which the Client finishes transmitting one MQTT Control Packet and the point it starts sending the next. It will be replaced by server keep alive returned from MQTT server.
...
- `{retry_interval, RetryInterval}`: Interval to retry sending packets that have been sent but not received a response. Defaults to 30s.

- `{will_topic, WillTopic}`: Topic of will message.

- `{will_payload, WillPayload}`: Payload of will message.

- `{will_retain, WillRetain}`: Whether the server should publish the will message as a retained message. Defaults to false.

- `{will_qos, WillQoS}`: QoS of will message. Defaults to 0.

- `{will_props, WillProperties}`: Properties of will message.

- `{auto_ack, boolean()}`: If true (the default), cliean process will automatically send ack packet like PUBACK when it receives a packet from the server. If false, application decides what to do.

- `{ack_timeout, AckTimeout}`: Maximum time to wait for a reply message. Defaults to 30s.

- `{force_ping, boolean()}`: If false (the default), if any other packet is sent during keep alive interval, the ping packet will not be sent this time. If true, the ping packet will be sent every time.

- `{low_mem, boolean()}`: If true, the client will try to reduce memory usage by garbage collecting more frequently. Defaults to false.

- `{reconnect, infinity | non_neg_integer()}`: The maximum number of reconnection attempts. Defaults to 0, means no reconnection.

- `{reconnect_timeout, pos_integer()}`: The time interval between reconnection attempts. Defaults to 5s.

- `{properties, Properties}`: Properties of CONNECT packet.

- `{custom_auth_callbacks, Callbacks}`: This configuration option enables enhanced authentication mechanisms in MQTT v5 by specifying custom callback functions.
...
- doc"""
Connect to the MQTT server over TCP or TLS and send a `CONNECT` packet with the options specified in `start_link/1, 2`.
`Client` must be a pid returned from `start_link/1, 2` or a name specified in `start_link/1, 2`.
""".
-spec(connect(client()) -> {ok, properties() | undefined} | {error, timeout | inet:posix()}).
connect(Client) ->
    call(Client, {connect, emqtt_sock}).

- doc"""
Connect to the MQTT server over Websocket and send a `CONNECT` packet with the options specified in `start_link/1, 2`.
`Client` must be a pid returned from `start_link/1, 2` or a name specified in `start_link/1, 2`.
""".
-spec(ws_connect(client()) -> {ok, properties() | undefined} | {error, timeout | inet:posix()}).
ws_connect(Client) ->
...
- doc"""
Send a `SUBSCRIBE` packet to the MQTT server. `Properties` specifies properties for `SUBSCRIBE` packet, defaults to `#{}` meaning no properties are attached.
`Subscriptions` specifies pairs of topic filter and subscription options. The topic filter is requried, the subscription options can be `[]`, equivalent to `[{rh, 0}, {rap, 0}, {nl, 0}, {qos, 0}]`.
""".
...
-doc"""
Send a `PUBLISH` packet to the MQTT server. `Topic`, `Properties` and `Payload` specify topic, properties and payload for `PUBLISH` packet.
`PubOpts` specifies qos and retain flag for `PUBLISH` packet, defaults to `[]`, equivalent to `[{qos, 0}, {retain, false}]`.
""".
...
-doc"""
Send a `UNSUBSCRIBE` packet to the MQTT server. `Properties` specifies properties for `SUBSCRIBE` packet, defaults to `#{}` meaning no properties are attached.
`Topics` specifies a list of topic filter with at least one topic filter.
""".
...
-doc """
Send a `PINGREQ` packet to the MQTT server. If `PINGRESP` packet is received from the server within the timeout period, `pong` is returned.
If not, `{error, ack_timeout}` is returned.
""".
-spec(ping(client()) -> pong | {error, ack_timeout}).
ping(Client) ->
...
-doc "See disconnect/3".
-spec(disconnect(client()) -> ok | {error, closed | inet:posix()}).
disconnect(Client) ->
    disconnect(Client, ?RC_SUCCESS).

-doc "See disconnect/3".
-spec(disconnect(client(), reason_code()) -> ok | {error, closed | inet:posix()}).
disconnect(Client, ReasonCode) ->
    disconnect(Client, ReasonCode, #{}).

-doc """
Send a `DISCONNECT` packet to the MQTT server. `ReasonCode` specifies a MQTT reason code for `DISCONNECT` packet, defaults to 0 meaning normal disconnection.
`Properties` specifies properties for `DISCONNECT` packet, defaults to `#{}` meaning no properties are attached.
""".
-spec(disconnect(client(), reason_code(), properties()) -> ok | {error, closed | inet:posix()}).
...
""".
-spec puback(client(), packet_id(), reason_code(), properties()) -> ok.
...
""".
-spec pubcomp(client(), packet_id(), reason_code(), properties()) -> ok.
...
-spec subscriptions(client()) -> [{topic(), [subopt()]}].
