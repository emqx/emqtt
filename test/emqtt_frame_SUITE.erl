...
t_parse_cont(_) ->
    ...
    {more, ContParse} = emqtt_frame:parse(<<>>, ParseState),
    {more, ContParse1} = emqtt_frame:parse(HdrBin, ContParse),
    {more, ContParse2} = emqtt_frame:parse(LenBin, ContParse1),
    {more, ContParse3} = emqtt_frame:parse(<<>>, ContParse2),
    {ok, Packet, <<>>, _} = emqtt_frame:parse(RestBin, ContParse3).
...
t_serialize_parse_v3_connect(_) ->
    ...
    {ok, Packet, <<>>, PState} = emqtt_frame:parse(Bin),
...
t_serialize_parse_v4_connect(_) ->
    ...
    ?assertMatch({ok, Packet, <<>>, _}, emqtt_frame:parse(Bin)).
...
t_parse_disconnect(_) ->
    ...
    ?assertMatch({ok, Packet, <<>>, _}, emqtt_frame:parse(<<224, 0>>)).
...
parse_serialize(Packet, Opts) when is_map(Opts) ->
    ...
    Bin = iolist_to_binary(emqtt_frame:serialize(Packet, Ver)),
    ParseState = emqtt_frame:initial_parse_state(Opts),
    {ok, NPacket, <<>>, _} = emqtt_frame:parse(Bin, ParseState),
    NPacket.

serialize_to_binary(Packet) ->
    iolist_to_binary(emqtt_frame:serialize(Packet)).

serialize_to_binary(Packet, Ver) ->
    iolist_to_binary(emqtt_frame:serialize(Packet, Ver)).

parse_to_packet(Bin, Opts) ->
    PState = emqtt_frame:initial_parse_state(Opts),
    {ok, Packet, <<>>, _} = emqtt_frame:parse(Bin, PState),
    Packet.
